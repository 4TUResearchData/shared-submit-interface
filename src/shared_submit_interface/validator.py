...
import re
from shared_submit_interface import convenience as conv
...
def string_value (record, field_name, minimum_length=0, maximum_length=None, required=False, error_list=None):
    ...
    if maximum_length is not None and conv.index_exists (value, maximum_length):
        return raise_or_return_error (error_list,
                    ValueTooLong(
                        field_name = field_name,
    ...
    if not conv.index_exists (value, minimum_length - 1):
        return raise_or_return_error (error_list,
                    ValueTooShort(
                        field_name = field_name,
