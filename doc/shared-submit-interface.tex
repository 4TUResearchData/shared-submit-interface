...
\usepackage{ssi}
\title{shared-submit-interface}
\author{Roel Janssen}
\begin{document}
\ifdefined\HCode
\ScriptEnv{html} {\NoFonts\hfill\break} {\EndNoFonts}
\Tag{TITLE+}{shared-submit-interface}
\CssFile
@font-face {
...
\begin{titlepage}
  \begin{center}
    \ifdefined\HCode
      \begin{html}
        <h1><code>Shared dataset submit interface</code></h1>
        <h2>a collaboration between 4TU.ResearchData and DANS KNAW.</h2>
      \end{html}
      \begin{html}
        <br><br>
      \end{html}
      version \ssiversion{}, \today{}~\\
    \else
      \topskip0pt
      \vspace*{\fill}
      { \Huge \t{shared-submit-interface} }~\\~\\
      { \Large A collaboration between 4TU.ResearchData and DANS KNAW.}~\\~\\
      ~\\~\\
